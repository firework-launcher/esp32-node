...
function trigger_firework(firework, launcher) {
...
function add_btns(rows, launcher) {
    ...
    for (let i = 1; i < rows+1; i++) {
        ...
        if (fireworks_launched[launcher].includes(i)) {
            button.classList.remove("disarmed");
...
function reload_all() {
    ...
    for (let index = 0; index < launchers.length; ++index) {
        if (launchers_armed[launchers[index]] == true) {
            document.getElementById("armbutton_" + launchers[index]).setAttribute("style", "display: none");
            document.getElementById("disarmbutton_" + launchers[index]).setAttribute("style", "");
        } else {
            document.getElementById("armbutton_" + launchers[index]).setAttribute("style", "");
            document.getElementById("disarmbutton_" + launchers[index]).setAttribute("style", "display: none");
        }
...
function node_crash_warning(data) {
...
for (let i = 0; i < Object.keys(root["launcher_data"]["names"]).length; i++) {
    add_node({
        "name": root["launcher_data"]["names"][Object.keys(root["launcher_data"]["names"])[i]],
