...
void ws2811_set_all(uint8_t green, uint8_t red, uint8_t blue) {
    uint32_t bits_to_send = (green << 16) | (red << 8) | blue;
    int32_t mask = 1 << (23);

    for (int bit = 0; bit < 24; bit++) {
        led_data[0 * 24 + bit].level0 = 1;
        led_data[0 * 24 + bit].level1 = 0;

        if ((bits_to_send & mask) == 0) {
            led_data[0 * 24 + bit].duration0 = T0H;
            led_data[0 * 24 + bit].duration1 = T0L;
        } else {
            led_data[0 * 24 + bit].duration0 = T1H;
            led_data[0 * 24 + bit].duration1 = T1L;
        }
        mask >>= 1;
    }

    memcpy(&led_data[24], &led_data[0], sizeof(rmt_item32_t) * 24 * (WS2811_LED_NUM - 1));
